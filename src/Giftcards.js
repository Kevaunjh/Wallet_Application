...
import giftcardsData from './GiftcardsData'; // Assuming 'amount' is the property name in this data
...
function Giftcards() {
  ...
  const [selectedCardAmount, setSelectedCardAmount] = useState('');
  ...
  const openModal = (barcodeImage, amount) => {
    ...
    setSelectedCardAmount(amount); // Pass the 'amount' to be displayed
    setIsModalOpen(true);
  };
  ...
  const giftcardContainers = filteredGiftcards.map((giftcard, index) => {
    ...
    return (
      <button 
        onClick={() => openModal(giftcard.barcodeImage, giftcard.amount)} // Pass the barcode and amount
        key={index}
